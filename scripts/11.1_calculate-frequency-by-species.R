...
sonoran.nativevolun <- bind_rows(sonoran.total.nativevolun, sonoran.wet.nativevolun, sonoran.dry.nativevolun,
  ...
  mutate(Plant = "Native recruit",
         Plot = c(rep("total", nrow(sonoran.total.nativevolun)), rep("wetter", nrow(sonoran.wet.nativevolun)),
                  rep("drier", nrow(sonoran.dry.nativevolun)), rep("very wet", nrow(sonoran.wettest.nativevolun)),
                  rep("very dry", nrow(sonoran.driest.nativevolun)))) 
...
sonoran.weedy <- bind_rows(sonoran.total.weedy, sonoran.wet.weedy, sonoran.dry.weedy,
  ...
  mutate(Plant = "Weed",
         Plot = c(rep("total", nrow(sonoran.total.weedy)), rep("wetter", nrow(sonoran.wet.weedy)),
                  rep("drier", nrow(sonoran.dry.weedy)), rep("very wet", nrow(sonoran.wettest.weedy)),
                  rep("very dry", nrow(sonoran.driest.weedy)))) 
...
sonoran.seed <- bind_rows(sonoran.total.seed, sonoran.wet.seed, sonoran.dry.seed,
  ...
  mutate(Plant = paste(mix, "mix"),
         Plot = c(rep("total", nrow(sonoran.total.seed)), rep("wetter", nrow(sonoran.wet.seed)),
                  rep("drier", nrow(sonoran.dry.seed)), rep("very wet", nrow(sonoran.wettest.seed)),
                  rep("very dry", nrow(sonoran.driest.seed)))) |> 
  select(-mix)
...
sonoran.empty <- bind_rows(sonoran.total.empty, sonoran.wet.empty, sonoran.dry.empty,
  ...
  mutate(Frequency = perc_freq / 100) |> 
  mutate(Plant = "Empty",
         Plot = c("total", "wetter", "drier", "very wet", "very dry"))
...
naz.nativevolun <- bind_rows(naz.total.nativevolun, naz.wet.nativevolun, naz.dry.nativevolun,
  ...
  mutate(Plant = "Native recruit",
         Plot = c(rep("total", nrow(naz.total.nativevolun)), rep("wetter", nrow(naz.wet.nativevolun)),
                  rep("drier", nrow(naz.dry.nativevolun)), rep("very wet", nrow(naz.wettest.nativevolun)),
                  rep("very dry", nrow(naz.driest.nativevolun)))) 
...
naz.weedy <- bind_rows(naz.total.weedy, naz.wet.weedy, naz.dry.weedy,
  ...
  mutate(Plant = "Weed",
         Plot = c(rep("total", nrow(naz.total.weedy)), rep("wetter", nrow(naz.wet.weedy)),
                  rep("drier", nrow(naz.dry.weedy)), rep("very wet", nrow(naz.wettest.weedy)),
                  rep("very dry", nrow(naz.driest.weedy)))) 
...
naz.seed <- bind_rows(naz.total.seedc, naz.wet.seedc, naz.dry.seedc,
  ...
  mutate(Plant = paste(mix, "mix"),
         Plot = c(rep("total", nrow(naz.total.seedc)), rep("wetter", nrow(naz.wet.seedc)),
                  rep("drier", nrow(naz.dry.seedc)), rep("very wet", nrow(naz.wettest.seedc)),
                  rep("very dry", nrow(naz.driest.seedc)),
                  rep("total", nrow(naz.total.seedp)), rep("wetter", nrow(naz.wet.seedp)),
                  rep("drier", nrow(naz.dry.seedp)), rep("very wet", nrow(naz.wettest.seedp)),
                  rep("very dry", nrow(naz.driest.seedp)))) |> 
  select(-mix)
...
present_species |> 
  ...
  print(n = 25)
#   missing from Pleasant: MUPO2, SPCR
#   missing from Preserve: MUPO2
#   missing from Roosevelt: MUPO2, SPCR
#   missing from SCC: DICA8, MUPO2
sonoran.seed1c.site <- present_species |> 
  filter(Region == "Sonoran Central",
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Current") |> 
  select(Site, Code) |> 
...
#   Appeared at no sites: BOCU, CAER
present_species |> 
  filter(Region == "Sonoran Central",
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Projected") |> 
  group_by(Site, Code) |> 
  summarise(Presence = n())
#   missing from Pleasant: BOAR, BORO2, BOCU, CAER
#   missing from Preserve: BOCU, CAER, ENFA
#   missing from Roosevelt: ARPU9, BOAR, BOCU, CAER
#   missing from SCC: BOAR, BOCU, CAER
sonoran.seed1p.site <- present_species |> 
  filter(Region == "Sonoran Central",
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Projected") |> 
  select(Site, Code) |> 
  ...
  mutate(Perc_sites = n / 4,
         Plant = "Projected mix") |> 
  filter(!Code %in% c("ARPU9", "BOCU", "SECO10"))


# Sonoran SE
# Current mix (7): BOGR2, ELEL5, HEMU3, HENE5, MATA2, POSE, SPCR
#   Appeared at no sites: HENE5
present_species |> 
  filter(Region == "Sonoran SE",
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Current") |> 
  group_by(Site, Code) |> 
  summarise(Presence = n())
#   missing from Patagonia: HEMU3, HENE5
#   missing from SRER: BOGR2, HENE5
sonoran.seed2c.site <- present_species |> 
  filter(Region == "Sonoran SE",
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Current") |> 
  select(Site, Code) |> 
...
#   Appeared at no sites: PLJA, PEPA8
present_species |> 
  filter(Region == "Sonoran SE",
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Projected") |> 
...
#   missing from SRER: PLJA, PEPA8
sonoran.seed2p.site <- present_species |> 
  filter(Region == "Sonoran SE",
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Projected") |> 
  select(Site, Code) |> 
...
# Make table for species at both sites
sonoran.seed3c.site <- present_species |> 
  filter(Region %in% c("Sonoran Central", "Sonoran SE"),
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Current",
         Code == "SPCR") |> 
  ...
  distinct(.keep_all = TRUE) |> 
  count(Code) |> 
  mutate(Perc_sites = n / 6,
         Plant = "Current mix")

sonoran.seed3p.site <- present_species |> 
  filter(Region %in% c("Sonoran Central", "Sonoran SE"),
         SpeciesSeeded == "Yes",
         PlotMix_Climate == "Projected",
         Code %in% c("ARPU9", "BOCU", "SECO10")) |> 
  select(Site, Code) |> 
  distinct(.keep_all = TRUE) |> 
  count(Code) |> 
  mutate(Perc_sites = n / 6,
...
sonoran.seed.site <- bind_rows(sonoran.seed1c.site, sonoran.seed1p.site, sonoran.seed2c.site,
                               sonoran.seed2p.site, sonoran.seed3c.site, sonoran.seed3p.site) |> 
  select(-n) |> 
