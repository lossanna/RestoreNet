...
# Purpose: Examine relationships between Count/Height and Since_last_precip.
...
all.des.count <- dat |> 
  filter(Weedy != "Weedy") |> 
  ggplot(aes(x = Since_last_precip, y = Count)) +
  ...
  xlab("Precip since last monitoring") +
...
all.des.height <- dat2 |> 
  filter(Weedy != "Weedy") |> 
  ggplot(aes(x = Since_last_precip, y = Height)) +
  geom_point(aes(color = PlantSource2,
                 shape = PlantSource2),
             alpha = 0.7) +
  geom_smooth() +
...
save.image("RData/06.5_exploratory-graphs_since-last-precip_subplot.RData")
